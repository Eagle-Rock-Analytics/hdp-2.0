...
- **`STATIONS_CSV_PATH`** — resolves to `s3://{HDP_STAGING_BUCKET}/2_clean_wx/temp_clean_all_station_list.csv`. This file is NOT regenerated by any pipeline script and is not durably provisioned for Phase 2 (`hdp-staging-pull` has a 30-day lifecycle expiration, so a manually-copied file will silently disappear). The old fallback path `s3://wecc-historical-wx/2_clean_wx/...` is also stale — that bucket's clean prefix was renamed to `2_clean_wx_zarr`. `QAQC_pipeline.py::run_qaqc_one_station()` now falls back to deriving `network` from the `{NETWORK}_{id}` era-id prefix if this CSV is missing, so single-station Phase 2 runs no longer hard-fail when it expires — but re-upload it (source: `hdp_documentation/Data/all_network_stationlist.csv`) if any code path still depends on the full CSV (e.g. `read_network_files`).
- **Static map assets (`WECC_TERR`/`WECC_MAR`/etc. in `paths.py`) have the same staging-bucket-TTL problem as `STATIONS_CSV_PATH`.** `qaqc_within_wecc` reads `s3://{HDP_STAGING_BUCKET}/0_maps/*.shp`; these shapefiles never change but are subject to `hdp-staging-pull`'s 30-day lifecycle expiration and silently disappear like any other staged artifact. If QAQC fails uniformly across stations at `qaqc_within_wecc` with an S3 "specified key does not exist" error, re-sync `s3://wecc-historical-wx/0_maps/` to `s3://{HDP_STAGING_BUCKET}/0_maps/`. This is a recurring class of bug (any code path using `BUCKET_NAME`/`HDP_STAGING_BUCKET` for genuinely static reference data will keep breaking every ~30 days) — the durable fix is to exempt static prefixes from the lifecycle rule or move them out of the TTL'd staging bucket entirely (tracked in `hdp-yxa`).
