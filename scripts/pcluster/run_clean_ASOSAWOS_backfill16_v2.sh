...
################################################################################
# SLURM Batch Script: run_clean_ASOSAWOS_backfill16_v2.sh
#
# Description:
#   Retries the ASOSAWOS clean stage for the 16 stations that still failed
#   after prior attempts. Uses inline bash array to avoid file lookup issues.
#   With expanded spot pool (c7i, m6i, m7i families), should have better
#   node availability and lower interruption risk.
#
# Stations: 16 ASOSAWOS backfill targets (GHCNh conversion validation failed in earlier runs)
# Strategy: Inline array lookup, aggressive retry on larger 8GB nodes (mem=7G request)
################################################################################

# Job Information:
#SBATCH --job-name=hdp-clean-bf16-v2
#SBATCH --array=1-16%2
#SBATCH --ntasks=1
#SBATCH --nodes=1
