...
This script performs data cleaning for the ASOS/AWOS network for ingestion into the Historical Observations Platform.
...
def merge_station_lists(key_asosawos: str, key_isd: str, cleandir: str) -> pd.DataFrame:
    ...
    index = asosawos_list[
        (asosawos_list.NCDCID == 20022657) & (np.isnan(asosawos_list.STARTDATE))
    ].index
    ...
    stationlist_join = isd_list.merge(
        asosawos_round,
        left_on=["WBAN", "LAT", "LON"],
        right_on=["WBAN", "LAT", "LON"],
        how="left",
    )
    add_rows = isd_list.merge(
            ...
            if df.empty is False:
                ...
                try:
                    ...
                    if station_metadata["Anemometer_elev"].isnull().all():
                        ...
                        anemometer_height = calc_clean._unit_elev_ft_to_m(
                            station_metadata["Anemometer_elev"].astype(float)
                        ).round(2)
                        ds = ds.assign_attrs(
                            anemometer_height_m=anemometer_height.values[0]
                        )

                    # Barometer height provided as elevation
                    if station_metadata["Barometer_elev"].isnull().all():
                        ds = ds.assign_attrs(barometer_elev_m=np.nan)
                    else:
                        barometer_height = calc_clean._unit_elev_ft_to_m(
                            station_metadata["Barometer_elev"].astype(float)
                        ).round(2)
                        ds = ds.assign_attrs(
                            barometer_elev_m=barometer_height.values[0]
                        )
                    ...
                    if "sfcWind" in ds:  # No conversions needed, do not add raw column.
                    ...
                    if "data_source" in ds:
                        ...
                        ds["data_source"].attrs[
                            "flag_meanings"
                        ] = "ASOSAWOS ASOSAWOS_merged_with_USAF_surface_hourly"

                    # drop any column that does not have any valid (non-nan data)
                    # need to keep elevation separate, as it can have a "valid" nan value, only drop if all other variables are also nans
                    for key in ds:
                        try:
                            if key != "elevation":
