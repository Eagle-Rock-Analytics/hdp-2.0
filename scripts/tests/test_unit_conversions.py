...
def test_pressure_conversion_hpa(grab_pressure_hpa):
    ...
    correct_conversion = grab_pressure_hpa * 100.0
    assert correct_conversion.equals(calc_clean_converted)
...
def test_pressure_conversion_kpa(grab_pressure_kpa):
    ...
    correct_coversion = grab_pressure_kpa * 1000.0
...
def test_moisture_conversion_kgkg(grab_moisture):
    ...
    correct_conversion = grab_moisture / 1000.0
    assert correct_conversion.equals(calc_clean_converted)
...
def test_longitude_conversion_dms(grab_longitude_dms):
    ...
    correct_conversion = -1 * (
        float(grab_longitude_dms[i][:3])
        + (float(grab_longitude_dms[i][4:6]) / 60)
        + (float(grab_longitude_dms[i][7:]) / 3600)
    )
    assert correct_conversion == (calc_clean_converted)
...
def test_longitude_conversion_dmm(grab_longitude_dmm):
    ...
    correct_conversion = -1 * (
        float(str(grab_longitude_dmm[i])[:3])
        + (float(str(grab_longitude_dmm[i])[3:]) / 60)
    )
    assert correct_conversion == (calc_clean_converted)
